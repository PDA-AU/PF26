    ...
    return (
        <div className="min-h-screen bg-muted">
            ...
            <nav className="bg-white border-b-2 border-black overflow-x-auto">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex gap-1">
                        <Link to="/admin" className="px-4 py-3 font-bold text-sm border-b-4 border-primary bg-secondary">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div className="flex items-center gap-4">
                            {stats?.registration_open ? (
                                ...
                                <PauseCircle className="w-10 h-10 text-red-500" />
                            )}
                            <div>
                        ...
                        <Button
                            ...
                            className={`${stats?.registration_open ? 'bg-red-500' : 'bg-green-500'} text-white border-2 border-black shadow-neo hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none`}
                            data-testid="toggle-registration-btn"
                        >
                            {stats?.registration_open ? (
                                <><PauseCircle className="w-5 h-5 mr-2" /> Pause Registration</>
                            ) : (
                                <><PlayCircle className="w-5 h-5 mr-2" /> Open Registration</>
                            )}
                        </Button>
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="neo-card">
                        <h3 className="font-heading font-bold text-lg mb-4 flex items-center gap-2">
                            <BarChart3 className="w-5 h-5" /> Gender Distribution
                        </h3>
                    ...
                    <div className="neo-card">
                        <h3 className="font-heading font-bold text-lg mb-4 flex items-center gap-2">
                            <TrendingUp className="w-5 h-5" /> Year Distribution
                        </h3>
                        <div className="space-y-3">
                    ...
                    <div className="neo-card">
                        ...
                        <div className="space-y-3">
                            <Button
                                ...
                                variant="outline"
                                className="w-full border-2 border-black shadow-neo hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none justify-start"
                ...
                <div className="neo-card">
                    <h3 className="font-heading font-bold text-lg mb-4 flex items-center gap-2">
                        <Users className="w-5 h-5" /> Department Distribution
                    </h3>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {stats?.department_distribution && Object.entries(stats.department_distribution).map(([dept, count]) => (
                            <div key={dept} className="flex items-center justify-between p-3 bg-muted border-2 border-black">
                                <span className="font-medium text-sm truncate max-w-[180px]">{dept}</span>
                                <span className="bg-primary text-white px-2 py-1 font-bold text-sm border-2 border-black ml-2">{count}</span>
                            </div>
                        ))}
                    </div>
                </div>
