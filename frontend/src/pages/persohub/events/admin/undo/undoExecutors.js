...
const panelDefinitionsRestore = async ({ eventSlug, command, getAuthHeader }) => {
    ...
    await axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/rounds/${roundId}/panels`, {
        panels,
    }, { headers: getAuthHeader() });
};
...
const roundPatchRestore = async ({ eventSlug, command, getAuthHeader }) => {
    ...
    await axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/rounds/${roundId}`, payload, {
        headers: getAuthHeader(),
    });
};
...
const roundFreezeRestore = async ({ eventSlug, command, getAuthHeader }) => {
    ...
    await axios.post(`${API}/persohub/admin/persohub-events/${eventSlug}/rounds/${roundId}/${endpoint}`, {}, {
        headers: getAuthHeader(),
    });
};
...
const eventFlagsRestore = async ({ eventSlug, command, getAuthHeader }) => {
    ...
    if (typeof command?.registration_open === 'boolean') {
        requests.push(axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/registration`, {
            registration_open: command.registration_open,
        }, { headers: getAuthHeader() }));
    }
    if (typeof command?.is_visible === 'boolean') {
        requests.push(axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/visibility`, {
            is_visible: command.is_visible,
        }, { headers: getAuthHeader() }));
    }
    if (
        typeof command?.results_published === 'boolean'
        || Object.prototype.hasOwnProperty.call(command || {}, 'results_caption')
        || Object.prototype.hasOwnProperty.call(command || {}, 'results_model_url')
    ) {
        requests.push(axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/results`, {
            ...
            results_model_url: command.results_model_url ?? null,
        }, { headers: getAuthHeader() }));
    }
...
const participantStatusBulkRestore = async ({ eventSlug, command, getAuthHeader }) => {
    ...
    if (!eventSlug) return;
    if (updates.length) {
        await axios.put(`${API}/persohub/admin/persohub-events/${eventSlug}/registrations/status-bulk`, {
            updates,
        }, { headers: getAuthHeader() });
    }
