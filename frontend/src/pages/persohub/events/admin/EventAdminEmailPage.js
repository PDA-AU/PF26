...
function EventAdminEmailContent() {
    ...
    const fetchUnregistered = useCallback(async () => {
        ...
        try {
            ...
            while (true) {
                ...
                params.append('page', String(pageIndex));
                params.append('page_size', String(pageSize));
                const response = await axios.get(`${API}/persohub/admin/persohub-events/${eventSlug}/unregistered-users?${params.toString()}`, {
                    headers: getAuthHeader(),
                });
                const data = Array.isArray(response.data) ? response.data : [];
                total = Number(response.headers['x-total-count'] || data.length || 0);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${API}/persohub/admin/persohub-events/${eventSlug}/email/bulk`, payload, { headers: getAuthHeader() });
            const info = res.data || {};
            setLastSendInfo({
                ...info,
                mode: effectiveMode,
