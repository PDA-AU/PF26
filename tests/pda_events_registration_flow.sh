...
ALL_CREATE="$(curl -fsS -X POST "$API/pda-admin/events" -H "Authorization: Bearer $SUPER_TOKEN" -H 'Content-Type: application/json' -d "{\"title\":\"Reg Flow All $STAMP\",\"description\":\"Registration flow all-college\",\"poster_url\":null,\"event_type\":\"Event\",\"format\":\"Offline\",\"template_option\":\"attendance_scoring\",\"participant_mode\":\"individual\",\"open_for\":\"ALL\",\"round_mode\":\"single\",\"round_count\":1,\"team_min_size\":null,\"team_max_size\":null,\"club_id\":1}")"
IND_SLUG="$(echo "$IND_CREATE" | parse_json 'data["slug"]')"
TEAM_SLUG="$(echo "$TEAM_CREATE" | parse_json 'data["slug"]')"
ALL_SLUG="$(echo "$ALL_CREATE" | parse_json 'data["slug"]')"
...
curl -fsS -X PUT "$API/pda-admin/events/$ALL_SLUG" -H "Authorization: Bearer $SUPER_TOKEN" -H 'Content-Type: application/json' -d '{"status":"open"}' >/dev/null
...
echo "[5/10] Set user2 as non-MIT"
curl -fsS -X PUT "$API/me" -H "Authorization: Bearer $USER2_TOKEN" -H 'Content-Type: application/json' -d '{"college":"Stanford"}' >/dev/null

echo "[6/10] MIT-only registration blocks non-MIT"
MIT_BLOCK_CODE="$(curl -sS -o /tmp/pda_mit_block_body.json -w '%{http_code}' -X POST "$API/pda/events/$IND_SLUG/register" -H "Authorization: Bearer $USER2_TOKEN" -H 'Content-Type: application/json' -d '{}')"
if [[ "$MIT_BLOCK_CODE" != "403" ]]; then
  echo "Expected MIT-only registration to block non-MIT user, got HTTP $MIT_BLOCK_CODE"
  cat /tmp/pda_mit_block_body.json
  exit 1
fi

echo "[7/10] ALL event allows non-MIT user"
curl -fsS -X POST "$API/pda/events/$ALL_SLUG/register" -H "Authorization: Bearer $USER2_TOKEN" -H 'Content-Type: application/json' -d '{}' >/dev/null

echo "[8/10] Individual registration path"
...
echo "[9/10] Team create + join by code"
...
echo "[10/10] Leader invite by regno"
...
echo "[11/10] Team dashboard entity status"
...
echo "[12/10] QR generation for individual and team"
...
echo "[13/10] My events endpoint reflects registrations"
