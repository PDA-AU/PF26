...
from badge_service import (
    ...
    delete_badges_for_persohub_event,
    delete_badges_for_persohub_event_team,
    delete_badges_for_persohub_event_user,
    delete_badges_for_persohub_teams,
    list_event_badges,
)
...
def delete_participant_with_cascade(
    ...
    delete_badges_for_persohub_event_user(db, event.id, user_id)
...
def delete_team_with_cascade(
    ...
    delete_badges_for_persohub_event_team(db, event.id, team_id)
    db.query(PersohubEventScore).filter(
        PersohubEventScore.event_id == event.id,
...
def update_round(
    ...
    if should_apply_shortlisting:
        latest_shortlist_round = (
            ...
            .filter(
                PersohubEventRound.event_id == event.id,
                PersohubEventRound.is_frozen == True,  # noqa: E712
                PersohubEventRound.state.notin_([PersohubEventRoundState.COMPLETED, PersohubEventRoundState.REVEAL]),
            )
            .order_by(PersohubEventRound.round_no.desc(), PersohubEventRound.id.desc())
            .first()
        )
        ...
        eligible_round_ids = [
            ...
            for row in (
                ...
                .filter(
                    ...
                    (
                        (PersohubEventRound.is_frozen == True)  # noqa: E712
                        | (PersohubEventRound.state == PersohubEventRoundState.COMPLETED)
                    ),
                )
                .all()
            )
        ...
        rounds_to_complete = (
            ...
            .filter(
                PersohubEventRound.event_id == event.id,
                PersohubEventRound.is_frozen == True,  # noqa: E712
                PersohubEventRound.round_no <= int(round_row.round_no),
                PersohubEventRound.state.notin_([PersohubEventRoundState.COMPLETED, PersohubEventRoundState.REVEAL]),
            )
        ...
        for row in rounds_to_complete:
            row.state = PersohubEventRoundState.COMPLETED
...
def export_leaderboard(
    ...
    event = _get_event_or_404(db, slug)
    round_rows_scope = (
        db.query(PersohubEventRound.id, PersohubEventRound.state, PersohubEventRound.is_frozen, PersohubEventRound.round_no)
        .filter(PersohubEventRound.event_id == event.id)
        .all()
    )
    ...
    round_meta = (
        ...
        .filter(
            PersohubEventRound.event_id == event.id,
            PersohubEventRound.id.in_(effective_round_ids) if effective_round_ids else text("1=0"),
        )
        .order_by(PersohubEventRound.round_no.asc(), PersohubEventRound.id.asc())
        .all()
    )
    ...
    if event.participant_mode == PersohubEventParticipantMode.INDIVIDUAL:
        ...
        if entity_ids and effective_round_ids:
            score_rows = (
                db.query(PersohubEventScore.user_id, PersohubEventScore.round_id, PersohubEventScore.normalized_score)
                .filter(
                    PersohubEventScore.event_id == event.id,
                    PersohubEventScore.entity_type == PersohubEventEntityType.USER,
                    PersohubEventScore.user_id.in_(entity_ids),
        ...
        if entity_ids and effective_round_ids:
            score_rows = (
                db.query(PersohubEventScore.team_id, PersohubEventScore.round_id, PersohubEventScore.total_score)
                .filter(
                    PersohubEventScore.event_id == event.id,
                    PersohubEventScore.entity_type == PersohubEventEntityType.TEAM,
                    PersohubEventScore.team_id.in_(entity_ids),
...
def create_badge(
    ...
    assignment = create_badge_assignment(
        ...
        user_id=payload.user_id,
        persohub_team_id=payload.team_id,
        persohub_event_id=event.id,
        meta={
            "title": payload.title,
    ...
    rows = list_event_badges(db, platform="persohub", event_id=event.id)
    ...
    return PersohubManagedBadgeResponse(
        ...
        team_id=created_assignment.persohub_team_id,
        created_at=created_assignment.created_at,
    )


@router.post("/persohub/admin/persohub-events/{slug}/badges/presign", response_model=PresignResponse)
def presign_badge_image_upload(
    ...
    presign = _generate_presigned_put_url(
        key_prefix=f"badges/persohub_events/{event.slug}",
...
@router.post("/persohub/admin/persohub-events/{slug}/badges/reveal-video/presign", response_model=PresignResponse)
def presign_badge_reveal_video_upload(
    ...
    event = _get_event_or_404(db, slug)
    presign = _generate_presigned_put_url(
        key_prefix=f"badges/persohub_events/{event.slug}/reveal-videos",
        filename=payload.filename,
        content_type=payload.content_type,
    ...
    for assignment, badge in rows:
        ...
        payload.append(
            PersohubManagedBadgeResponse(
                ...
                team_id=assignment.persohub_team_id,
