...
def event_dashboard(
    ...
    if user_ids:
        user_rows = (
            db.query(
                ...
                func.count(PdaEventScore.id).label("row_count"),
                func.count(PdaEventScore.id).filter(PdaEventScore.is_present == True).label("present_count"),  # noqa: E712
            )
            .filter(
                PdaEventScore.event_id == event.id,
    ...
    if user_ids:
        user_present_rows = db.query(PdaEventAttendance.user_id).filter(
    ...
    if team_ids:
        team_present_rows = db.query(PdaEventAttendance.team_id).filter(
            PdaEventAttendance.event_id == event.id,
...
def event_attendance(
    ...
    if round_id is not None:
        ...
        score_rows = db.query(PdaEventScore).filter(
            PdaEventScore.event_id == event.id,
            PdaEventScore.round_id == int(round_id),
        ).all()
    else:
...
def mark_attendance(
    ...
    if level == "round":
        row = db.query(PdaEventScore).filter(
            PdaEventScore.event_id == event.id,
            PdaEventScore.round_id == int(payload.round_id),
            PdaEventScore.entity_type == entity_type,
            PdaEventScore.user_id == payload.user_id,
            PdaEventScore.team_id == payload.team_id,
        ).first()
        if row:
...
@router.get("/pda-admin/events/{slug}/export/attendance")
        ...
        if ordered_rounds:
            round_row_map = {
                ...
                for row in (
                    ...
                    .filter(
                        PdaEventRound.event_id == event.id,
                        PdaEventRound.id.in_([round_id for round_id, _round_no in ordered_rounds]),
                    )
                    .all()
                )
            }
