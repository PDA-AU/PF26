...
class PersohubEvent(Base):
    ...
    persohub_access_requested_at = Column(DateTime(timezone=True), nullable=True)
    persohub_access_requested_by_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    persohub_access_reviewed_at = Column(DateTime(timezone=True), nullable=True)
    persohub_access_reviewed_by_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class PersohubEventRegistration(Base):
    ...
    eliminated_round_no = Column(Integer, nullable=True)
...
class PersohubClub(Base):
    ...
    persohub_events_access_status = Column(String(16), nullable=False, default="rejected", server_default="rejected")
    persohub_events_access_requested_at = Column(DateTime(timezone=True), nullable=True)
    persohub_events_access_requested_by_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    persohub_events_access_reviewed_at = Column(DateTime(timezone=True), nullable=True)
    persohub_events_access_reviewed_by_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    persohub_events_access_review_note = Column(Text, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
